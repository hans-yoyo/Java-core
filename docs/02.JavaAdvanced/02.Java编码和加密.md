...
Man是三个 字符，一共24个有效bit，用4个Base64字符来凑齐24个有效位。红框表示的是对应的Base64，6个有效位转化成相应的索引值再对应 Base64字符表，查出"Man"对应的Base64字符是"TWFU"。说到这里有个原则不知道你发现了没有，要转换成Base64的最小单位就是三个字节，对一个字符串来说每次都是三个字节三个字节的转换，对应的是Base64的四个字节。

![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190602165510.png)
...
![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190602165947.png)
...
eyJ1aWQiOiI5NkFDRjVDMi1CQTI0LTExRTgtODY5Mi0yOEQyNDRERTlDQTUiLCJleHAiOjE1MzkzNzAwNzN9.   ----->{"uid":"96ACF5C2-BA24-11E8-8692-28D244DE9CA5","exp":1539370073}

ypdwiaPzKZ-_EqbAlwa-jtmECKH4-_9e4i7f_HiiDxU
```

解密eyJ0

```xml
eyJ0: 30 50 9 52 (BASE64码表)
0001 1110 | 0011 0010 | 1001 | 0011 0100
01 1110 | 11 0010 | 00 1001 | 11 0100
01 1110 11 | 0010 0010 | 0111 0100
123 34 116(ASCII码表)
{    "   t
```
...
一种是对明文的单个位（或字节）进行运算，称为流密码，也称为**序列密码**；

一种是把明文信息划分为不同的组（或块）结构，分别对每个组（或块）进行加密、解密，称为**分组密码**。

![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190602163503.png)

假设甲乙方作为通信双方。假定甲乙双方在消息传递前已商定加密算法，欲完成一次消息传递需要经过如下步骤。

![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190602163603.png)
...
根据数据加密时每个加密区块见得关联方式来区分，可以分为以下几种工作模式：
...
![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190602171622.png)
...
![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190602171653.png)
