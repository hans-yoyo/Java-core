...
![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190601113036.png)
...
- 静态方法不可以定义**this, super**关键字

静态方法常被用于各种工具类、工厂方法类。

**总结**

* 随着类的加载而加载(可直接通过类名直接调用方法、属性）
* 优先于对象存在
* 被类的所有对象共享
* 可以通过类名调用
...
- 位置 - 类中，一般在构造方法前
- 作用 - 把构造方法中相同的内容定义到构造代码块里，提高代码复用性
...
public class Demo6 {
     public static void main(String[] args) {
           Bird b1 = new Bird();
           Bird b2 = new Bird("燕子","雄鸟");
           Bird b3 = new Bird("麻雀","雄鸟",10);
...
class Bird{
     ...
     //作用：调用构造方法之前，会先执行构造代码块的内容
     {
           System.out.println("我是构造代码块");
     }
     
     public Bird(){//构造方法没有返回值（不能写void），但是方法体内可以写return
...
覆写/重写（Override）是指子类根据需求对从父类继承的方法进行重新编写
...
![](https://raw.githubusercontent.com/hyman213/FigureBed/master/2019/06/20190601115547.png)
